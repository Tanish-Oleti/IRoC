...
class differential_ik:
    ...
    def joy_callback(self,data):
        ...
        self.y_vel=self.joy_array[1]*factor
        self.z_vel=self.joy_array[2]*factor

    def joint_angles_callback(self,data):
        self.joint_angles=data
        base_angle=self.joint_angles[0] #base is phi
        shoulder_angle=-self.joint_angles[1]+90 #shoulder is theta1
        elbow_angle=self.joint_angles[2]+90 #elbow is theta2
...
def main(args):
    td=differential_ik()
    rospy.init_node('differential_ik',anonymous=True)
    try:
        rospy.spin()
